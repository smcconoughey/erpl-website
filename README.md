...
Datanator is a Vite app. The live service settings must match `render.yaml`:

- Build command: `cd subdomains/data && npm ci && npm run build`
- Publish directory: `./subdomains/data/dist`

Update those two fields in the Render dashboard before merging this app to
`main`. The Blueprint has not been applied and will not change the existing
service. Do not create another service from the Blueprint as an activation
step. The existing main website service is managed separately.
